...
PKGNAME_LIBICONV="libiconv-1.17"
PKGNAME_FLAC="flac-1.3.4"
PKGNAME_ZLIB="zlib-1.3.1"
...
#
mkdir gcc && cd gcc && \
CC="$GCC_CC" CXX="$GCC_CXX" ../../buffaloam/"$PKGNAME_GCC"/configure --prefix="$CROSS64_PATH" --target=x86_64-w64-mingw32 --disable-multilib --enable-languages=c,c++ && \
...
PKG_CONFIG_PATH="$CROSS64_PATH/lib/pkgconfig" CFLAGS="-O2 -g -fno-exceptions -fno-asynchronous-unwind-tables" CPPFLAGS="-I$CROSS64_PATH/include" LDFLAGS="-L$CROSS64_PATH/lib" \
        ../../buffaloam/"$PKGNAME_FLAC"/configure --prefix="$CROSS64_PATH" --host=x86_64-w64-mingw32 --disable-shared --enable-static --disable-ogg --disable-oggtest && \
make -j4 && make install && \
cd .. && \
